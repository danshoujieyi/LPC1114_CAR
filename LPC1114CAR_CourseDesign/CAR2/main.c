...
static rt_uint8_t track_move_thread_stack[1536];
static rt_uint8_t oled_display_thread_stack[512];
...
// 新增：接收准备指令的标志（独立，避免冲突）
static uint8_t recv_car1_ready_flag = 0;
// 新增：二号车出库完成标志（可选，用于出库后停在起点等待）
static uint8_t car2_out_garage_flag = 0;

// 新增：二号→三号的准备指令（通知三号做好准备）
#define CAR3_READY_CMD 0xBA      
// 新增：二号→三号的开始指令（通知三号开始运动）
#define CAR3_START_CMD 0xBB    

// 新增：发送三号准备指令的标志（避免重复发送）
static uint8_t send_car3_ready_flag = 0;  

// 通信控制标志（避免重复接收/发送）
static uint8_t recv_car1_flag = 0;    // 1=已接收一号小车指令，0=未接收
static uint8_t send_to_car3_flag = 0; // 1=已向三号发送指令，0=未发送
...
#define INIT_FORWARD_AFTER_BLACK_MS 220  // 找到黑线后再前进200ms
...
#define SEND_CAR3_DELAY_MS  500  
// 新增：计时相关静态变量（仅在该函数内有效，保留值）
static uint8_t  car3_timer_start_flag = 0; // 计时启动标志（0=未启动，1=已启动）
static rt_tick_t car3_start_tick = 0;      // 计时起始时间戳（RT-Thread节拍）

// 新增：发送三号准备指令的延时（单位：ms，根据需求调整，比如500ms）
#define DETECT_DELAY_MS  2000  
// 新增：计时相关静态变量（仅在该函数内有效，保留值）
static uint8_t detect_timer_start_flag = 0; // 计时启动标志（0=未启动，1=已启动）
...
//				if (send_car3_ready_flag == 0) {
//					
//					send_car3_ready_flag = 1; // 标记待发送
					
					detect_start_tick = rt_tick_get();
					recv_car1_flag = 2; // 标记已触发，避免重复切换
...
//				}
        ...
        case STATE_STOP_INIT:
            ...
            if (key_flag) {  // 检测到按键按下
                key_flag = 0;
//				car3_timer_start_flag = 1;    
//				car3_start_tick = rt_tick_get(); 
...
static void comm_thread_entry(void* parameter)
{
    ...
    while (1)
    {
		...
		if (ReceiveFlag == 1 && recv_car1_ready_flag == 0) {  // 接收成功 + 未处理过
            ReceiveSuccessCount++;
            // 检测到一号车的准备指令（0xAD）
            if (NRF24L01_RxPacket[0] == CAR2_READY_CMD) {
                recv_car1_ready_flag = 1;  // 标记已接收，避免重复
                // 触发二号车“出车库”逻辑（比如设置出库标志/模拟按键）
                // 若你已有出库状态（如STATE_OUT_GARAGE），可直接设状态：
                // current_state = STATE_OUT_GARAGE;
                // 简易版：用标志位触发出库流程
                car2_out_garage_flag = 1;
                // OLED反馈
        ...
        if (ReceiveFlag == 1 && recv_car1_flag == 0) {  // 接收成功且未处理过
            ReceiveSuccessCount++;
            // 检测到一号小车的启动指令（0x01）
            if (NRF24L01_RxPacket[0] == CAR2_START_CMD) {
                recv_car1_flag = 1;          // 标记已接收，避免重复触发
        ...
        if (send_to_car3_flag == 1)
        {
            // 构造给三号小车的启动指令（0x02）
            NRF24L01_TxPacket[0] = 0x00;
            NRF24L01_TxPacket[1] = CAR3_START_CMD;
            NRF24L01_TxPacket[2] = 0x00;
            NRF24L01_TxPacket[3] = 0x00;
            ...
            if (SendFlag == 1) {            // 发送成功
                SendSuccessCount++;
                send_to_car3_flag = 1;     // 标记已发送，避免重复
...
static void oled_display_entry(void* parameter)
{
    ...
    OLED_ShowString(1, 1, "Team 13 Car - 2");  // 首行：第十三组 二号小车
